...
export function CollectionPage() {
  ...
  const fetchDesigns = async () => {
    ...
    try {
      ...
      // Robustly fetch recommended designs matching current category
      let recList: Design[] = [];
      const recParams: any = { limit: 24, sort: 'newest' };
      if (category && category !== 'All') {
        recParams.category = category;
      }
      ...
      // Strictly only recommend designs belonging to current category
      const filteredRecs = recList.filter((d: any) => {
        const id = getDesignId(d);
        const matchesCategory =
          category === 'All' ||
          !category ||
          (d.category && d.category.trim().toLowerCase() === category.trim().toLowerCase());
      ...
      if (subcategory && subcategory !== 'All') {
        filteredRecs.sort((a: any, b: any) => {
          const aSub = (a.subcategory || '').toLowerCase() === subcategory.toLowerCase() ? 1 : 0;
          const bSub = (b.subcategory || '').toLowerCase() === subcategory.toLowerCase() ? 1 : 0;
          return bSub - aSub;
        });
      }
